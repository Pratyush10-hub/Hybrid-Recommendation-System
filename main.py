...
def perform_sentiment_analysis(input_file="appliances.json", output_file="appliances_with_sentiment.parquet"):
    """
    Loads JSONL data, performs BERT-based sentiment analysis, and saves enriched data.
    """
    print("="*60)
    ...
    print("\n[2/6] Loading BERT sentiment model...")
    ...
    # Sentiment Score (0–5) + Confidence (0–1)
    def get_sentiment_score_and_confidence(text_list):
        ...
        # Expected sentiment score in [1,5]
    ...
    # Apply Sentiment Analysis (Batched)
    ...
    df['SentimentScore'] = sent_scores
    df['SentimentConfidence'] = confidences
...
class MFAdam:
    ...
    def step(self, t):
        ...
        for idx, (u, i) in enumerate(zip(self.users, self.items)):
            ...
            # Update biases
            for p, g, m, v, idx_p in [(self.bu, g_bu, self.mbu, self.vbu, u), 
                                       (self.bi, g_bi, self.mbi, self.vbi, i)]:
            ...
            # Update matrices
            for mat, grad, m_mat, v_mat, idx_mat in [(self.U, g_U, self.mU, self.vU, u), 
                                                       (self.V, g_V, self.mV, self.vV, i)]:
...
# ==============================
# PART 3: FUZZY HYBRID SYSTEM
# ==============================

class FuzzyHybridRecommender:
    """Hybrid Recommender combining MF and Fuzzy Inference System"""
...
def train_hybrid_recommender(data_file="appliances_with_sentiment.parquet"):
    """
    Trains the complete hybrid recommendation system
    """
    print("\n" + "="*60)
    print("PART 2: Hybrid Recommender Training")
    print("="*60)
    
    # Load & Prepare Data
    print("\n[1/9] Loading preprocessed data...")
    data = pd.read_parquet(data_file).dropna(subset=['overall', 'SentimentScore', 'SentimentConfidence'])
    ...
    # Split train/val
    print("\n[2/9] Splitting data into train/validation...")
    ...
    # Build Sparse Train Matrices
    print("\n[3/9] Building sparse matrices...")
    ...
    # Tune MF Models
    print("\n[4/9] Tuning Matrix Factorization for Ratings...")
    ...
    print(f"✓ Best rating params: {best_rating_params}")
    
    print("\n[5/9] Tuning Matrix Factorization for Sentiment...")
    best_sentiment_params = tune_mf(S_train, train, 'SentimentNorm', trials=30)
    print(f"✓ Best sentiment params: {best_sentiment_params}")
    
    # Train final MF models
    print("\n[6/9] Training final MF models...")
    mf_ratings = MFAdam(R_train, **best_rating_params)
    mf_ratings.fit()
    mf_sentiment = MFAdam(S_train, **best_sentiment_params)
    mf_sentiment.fit()
    print("✓ MF models trained")
    
    # Save MF models
    joblib.dump(mf_ratings, 'mf_ratings.pkl')
    joblib.dump(mf_sentiment, 'mf_sentiment.pkl')
    print("✓ Saved MF models")
    
    # Build Fuzzy Logic System
    print("\n[7/9] Building Fuzzy Logic System...")
    ...
    # Build fuzzy rules from training predictions
    ...
    # Tune Fuzzy Hybrid Parameters
    print("\n[8/9] Tuning fuzzy hybrid parameters...")
    ...
    print(f"✓ Best fuzzy params: {best_fuzzy_params}")
    
    # Create final hybrid model
    hybrid_model = FuzzyHybridRecommender(
        mf_ratings, mf_sentiment, fuzzy_system, 
        best_fuzzy_params['alpha'], best_fuzzy_params['bias_shift']
    )
    
    # Evaluate on Validation Set
    print("\n[9/9] Evaluating models on validation set...")
    def print_metrics(name, true, preds):
        ...
        print(f"{name:25s}: RMSE={rmse:.4f}, MAE={mae:.4f}, Prec={prec:.4f}, Rec={rec:.4f}, F1={f1:.4f}")
    ...
    return hybrid_model, mf_ratings, mf_sentiment, fuzzy_system
...
def main():
    ...
    """
    print("\n" + "="*60)
    print("FUZZY HYBRID RECOMMENDATION SYSTEM")
    print("="*60)
    
    # Step 1: Sentiment Analysis
    try:
        df = perform_sentiment_analysis(
            input_file="appliances.json",
            output_file="appliances_with_sentiment.parquet"
        )
    except FileNotFoundError:
        print("\n⚠ Warning: appliances.json not found. Skipping sentiment analysis.")
        print("Attempting to use existing appliances_with_sentiment.parquet...")
    
    # Step 2: Train Hybrid Recommender
    hybrid_model, mf_ratings, mf_sentiment, fuzzy_system = train_hybrid_recommender(
        data_file="appliances_with_sentiment.parquet"
    )
    
    print("\n✅ Pipeline complete! Models saved:")
    print("   - mf_ratings.pkl")
    print("   - mf_sentiment.pkl")
    print("\nYou can now use these models for recommendations!")
